...
public class HeadquarterDao {
	...
	public List<HeadquarterListDto> headquarterList(String state_id, int headquarter_id, String start, String end) {
		...
		{if(flag==0) {
			qry = "select ds.headquarter_id, h.headquarter_name, sum("+query_str+") as headquarter_total_sale from doctor d\r\n"
					...
					+ " and docs.year="+year+" GROUP BY ds.headquarter_id";
		}else
			if(flag==1)
			{
				qry = "SELECT headquarter_id,headquarter_name, SUM(headquarter_total_sale) AS headquarter_total_sale\r\n" + 
						"FROM\r\n" + 
						" ( \r\n" + 
						"select ds.headquarter_id, h.headquarter_name, sum("+start_query_str+") as headquarter_total_sale from doctor d join division_state ds on ds.division_state_id = d.division_state_id join doctor_medicine dm on dm.doctor_id = d.doctor_id join doctor_sale docs on docs.doctor_medicine_id = dm.doctor_medicine_id join headquarter h on h.headquarter_id = ds.headquarter_id where ds.state_id= "+ state_id+" and docs.year="+startYear+"  GROUP BY ds.headquarter_id\r\n" + 
						"union \r\n" + 
						"select ds.headquarter_id, h.headquarter_name, sum("+end_query_str+") as headquarter_total_sale1 from doctor d join division_state ds on ds.division_state_id = d.division_state_id join doctor_medicine dm on dm.doctor_id = d.doctor_id join doctor_sale docs on docs.doctor_medicine_id = dm.doctor_medicine_id join headquarter h on h.headquarter_id = ds.headquarter_id where ds.state_id= "+ state_id+" and docs.year="+endYear+"  GROUP BY ds.headquarter_id\r\n" + 
			...
			qry = "select ds.headquarter_id, h.headquarter_name , sum("+query_str+") as headquarter_total_sale from doctor d\r\n"
					...
					+ "join doctor_sale docs on docs.doctor_medicine_id = dm.doctor_medicine_id\r\n"
					+ "join headquarter h on h.headquarter_id = ds.headquarter_id\r\n" + "where ds.state_id=" + state_id
					+ " and ds.headquarter_id=" + headquarter_id + " and docs.year="+year+" GROUP BY ds.headquarter_id";
