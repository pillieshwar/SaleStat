...
public class HeadquarterDao {
	...
	public List<HeadquarterListDto> headquarterList(String state_id, int headquarter_id) {
		...
		{
			qry = "select ds.headquarter_id, h.headquarter_name, sum(coalesce(jan_sale,0)+coalesce(feb_sale,0)+coalesce(mar_sale,0)+coalesce(apr_sale,0)+coalesce(may_sale,0)+coalesce(jun_sale,0)+coalesce(jul_sale,0)+coalesce(aug_sale,0)+coalesce(sep_sale,0)+coalesce(oct_sale,0)+coalesce(nov_sale,0)+coalesce(dec_sale,0)) as headquarter_total_sale from doctor d\r\n"
			...
			qry = "select ds.headquarter_id, h.headquarter_name , sum(coalesce(jan_sale,0)+coalesce(feb_sale,0)+coalesce(mar_sale,0)+coalesce(apr_sale,0)+coalesce(may_sale,0)+coalesce(jun_sale,0)+coalesce(jul_sale,0)+coalesce(aug_sale,0)+coalesce(sep_sale,0)+coalesce(oct_sale,0)+coalesce(nov_sale,0)+coalesce(dec_sale,0)) as headquarter_total_sale from doctor d\r\n"
					...
					+ "join doctor_sale docs on docs.doctor_medicine_id = dm.doctor_medicine_id\r\n"
					+ "join headquarter h on h.headquarter_id = ds.headquarter_id\r\n" + "where ds.state_id=" + state_id
					+ " and ds.headquarter_id=" + headquarter_id + " GROUP BY ds.headquarter_id";
